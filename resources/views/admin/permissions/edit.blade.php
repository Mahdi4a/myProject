                ...
                <form class="form-horizontal" method="POST" action="{{ route('admin.permissions.update' , $permission->id) }}">
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <input type="text" name="name" value="{{ (old('name') ?? $permission->name) ?? "" }}" class="form-control" id="inputName"
                                   placeholder="عنوان دسترسی را وارد کنید">
                        </div>
                        <div class="form-group">
                            <label for="inputLabel" class="col-sm-2 control-label">توضیح دسترسی</label>
                            <input type="text" name="label" value="{{ (old('label') ?? $permission->label) ?? "" }}" class="form-control" id="inputLabel"
