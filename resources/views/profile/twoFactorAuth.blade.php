...
@section('main')
    ...
    <form action="{{route('two.factor.auth')}}" method="POST">
        ...
        <div class="form-group">
            ...
            <select name="type" id="type" class="form-control">
                @foreach(config('twoFactor.type') as $key => $name)
                    <option value="{{$key}}" {{auth()->user()->two_factor_type === $key || old('type') === $key ? "selected":""}}>{{$name}}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <input type="text" name="phone" id="phone" value="{{old('phone') ?? auth()->user()->phone_number}}" class="form-control">
        </div>
