@component('main::admin.layouts.content',['title' => 'ایجاد کد تخفیف'])
    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                ...
                <!-- /.card-header -->
                <!-- form start -->
                <form class="form-horizontal" method="POST" action="{{ route('admin.discount.store') }}">
                    @csrf
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <select name="users[]" id="users" multiple="multiple" class="form-control selectSearch">
                                @foreach($users as $user)
                                    <option
                                        value="{{$user->id}}" {{ in_array($user->id, old('users' , []), true) ? 'selected' : ''  }} >{{$user->name}}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select name="products[]" id="products" multiple="multiple"
                                ...
                                @foreach($products as $product)
                                    <option
                                        value="{{$product->id}}" {{ in_array($product->id, old('products' , []), true) ? 'selected' : ''  }} >{{$product->name}}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select name="categories[]" id="categories" multiple="multiple"
                                ...
                                @foreach($categories as $category)
                                    <option
                                        value="{{$category->id}}" {{ in_array($category->id, old('categories' , []), true) ? 'selected' : ''  }} >{{$category->name}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
