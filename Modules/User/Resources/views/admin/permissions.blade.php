@component('main::admin.layouts.content',['title' => 'ثبت دسترسی'])
                        ...
                        <div class="form-group {{ $errors->has('roles') ? 'has-error' : ''}}">
                            ...
                            <select name="roles[]" id="roles" multiple="multiple" class="form-control">
                                @foreach($roles as $role)
                                    <option
                                        value="{{$role->id}}" {{ in_array($role->id, $user->roles->pluck('id')->toArray(), true) ? 'selected' : ''  }} >{{$role->name}}
                                        - {{$role->label}}</option>
                                @endforeach
                            </select>
                            {!! $errors->first('roles', '<p class="help-block" style="color:red">:message</p>') !!}
                        </div>
                        <div class="form-group {{ $errors->has('permissions') ? 'has-error' : ''}}">
                            ...
                            <select name="permissions[]" id="permissions" multiple="multiple" class="form-control">
                                @foreach($permissions as $permission)
                                    <option
                                        value="{{$permission->id}}" {{ in_array($permission->id, $user->permissions->pluck('id')->toArray(), true) ? 'selected' : ''  }} >{{$permission->name}}
